...
import { getAllRequestUnverified } from "@/lib/supabase/requestService";
// NOTE: Make sure suspendUser is imported from your service file
import { sendMessageToUser, suspendUser } from "@/lib/supabase/userService"; 
...
export default function PendingVerifications() {
  ...
  // State for Message Dialog (Existing)
  ...
  const suspendMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setSelectedRenter(null);
      // Invalidate the query to refresh the list/status
  ...
  const { data: renters, isLoading, error } = useQuery({
    ...
    queryFn: getAllRequestUnverified,
  });
  ...
  const handleOpenSuspendDialog = (renter: RenterData) => {
    ...
    setSuspensionDuration("3d"); // Reset to default duration
  };
  ...
  // --- Rendering Conditional States (Unchanged) ---
  // if (isLoading)
  //   return (
  //     <div className="flex items-center justify-center py-12 text-primary">
  //       <Loader2 className="h-6 w-6 animate-spin mr-2" /> Loading renters...
  //     </div>
      ...
      <Dialog open={isSuspendDialogOpen} onOpenChange={setIsSuspendDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              You are about to suspend the account of **{selectedRenter?.users?.firstname || 'the user'}**. Select the duration below.
            </DialogDescription>
          </DialogHeader>
