...
// NOTE: Must ensure 'suspendUser' is imported from your userService file
import { rejectLandlordProof, sendMessageToUser, updateLandlordStatus, suspendUser } from "@/lib/supabase/userService" 
import { toast } from "sonner"
import { cn } from "@/lib/utils"
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from "./ui/dropdown-menu"
import { Input } from "@/components/ui/input"
import { IconCheck, IconX, IconBan } from "@tabler/icons-react" // Ensure Ban icon is available
 ...
 return (
        <div className="w-full flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between gap-4 lg:px-2">
                ...
                {/* Right Side: Customize Columns & Add Section (UNCHANGED) */}
                <div className="flex items-center space-x-2">
                    
                    {/* Customize Columns Dropdown (UNCHANGED) */}
                    <DropdownMenu>
  ...
  >
   {row.getVisibleCells().map((cell, index) => (
    <TableCell 
        ...
        className={cn(
            "pr-4 py-3",
            // Styling applies to the first cell (index 0)
            index === 0 && "font-semibold text-foreground"
        )} 
        onClick={() => onSelectUser(row.original)} // Clicking the data cell opens the drawer
    >
        {index === 0 ? ( 
            // 🎯 FIX: Displaying the ID with hover underline classes
 ...
 const verifyMutation = useMutation({
  ...
  const suspendMutation = useMutation({
    ...
    onSuccess: () => {
        ...
        // Invalidate relevant queries to update status in the table
        queryClient.invalidateQueries({ queryKey: ["users"] });
        // Optionally close the drawer if suspension is the final action
        onClose(); 
    },
    onError: (error: any) => {
 ...
 return (
  <Drawer direction="right" open={true} onOpenChange={(open) => { if (!open) onClose(); }}>
   <DrawerContent className="p-0">
    ...
    {/* Footer Logic for Verification vs. Suspension */}
